...
- `restart: unless-stopped`, port `1337` published, `env_file: cms/.env`, and a **bind mount** (not a named volume) for the SQLite data directory — e.g. `./cms-data:/opt/app/database` mapped to wherever `config/database.ts` points SQLite. A bind mount rather than a named volume is the deliberate choice here: it's what makes "spin it up wherever I need it" actually easy — the whole CMS state is one plain folder that can be rsynced/backed up/moved to a different Docker host without touching Docker's internal volume storage.
